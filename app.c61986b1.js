...
var formToAddItemsID = document.getElementById('form');
var listOfItemsID = document.getElementById('tasklist');
var input = document.getElementById('input');
var parsingLocalStorageItems = JSON.parse(localStorage.getItem('items') || "[]");
...
function populateList(itemArray, itemList) {
  itemList.innerHTML = itemArray.map(function (item, i) {
    return "\n\t\t  <li>\n\t\t  <div>\n\t\t  <input type=\"checkbox\" data-index=".concat(i, " id=\"item").concat(i, "\" ").concat(item.checkedOff ? 'checked' : '', " />\n\t\t  <label class=\"label\" data-content=\"").concat(item.itemText, "\" for=\"item").concat(i, "\">").concat(item.itemText, "</label>\n\t\t  </div>\n\t\t  <span class=\"close\" data-index=").concat(i, "  id=\"removed").concat(i, "\" ").concat(item.deleted ? true : false, ">\u2716</span>\n\t\t  </li>\n\t ");
  }).join('');
}
